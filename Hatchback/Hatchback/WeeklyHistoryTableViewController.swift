//
//  WeeklyHistoryTableViewController.swift
//  Hatchback
//
...
class WeeklyHistoryTableViewController: PFQueryTableViewController {
    ...
    override func tableView(_: UITableView, cellForRowAt indexPath: IndexPath, object: PFObject?) -> PFTableViewCell? {
        ...
        var dashCell = tableView.dequeueReusableCell(withIdentifier: "weekCell") as! WeeklyHistoryCell!
        
        if dashCell == nil {
            dashCell = WeeklyHistoryCell(style: UITableViewCellStyle.default, reuseIdentifier: "weekCell")
        }
        ...
        if let trip = object {
            ...
            if minsDriven > 59 {
                ...
                if hoursDriven > 1 {
                    dashCell?.timeLabel.text = "\(stringFromDate.lowercased()) - \(hoursDriven) hrs \(minsDriven) minutes"
                } else {
                    dashCell?.timeLabel.text = "\(stringFromDate.lowercased()) - \(hoursDriven) hr \(minsDriven) minutes"
                }
                
            } else {
                dashCell?.timeLabel.text = "\(stringFromDate.lowercased()) - \(minsDriven) minutes"
            }
            //            let hoursDriven:TimeInterval = minutesDriven / 60
            
            //setting the percentage label
            let percentage = trip["drivePercentage"] as! Double
            dashCell?.percentage.text = "\(Int(percentage))%"
